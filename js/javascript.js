(function () {
    function arrayForBar(size) {
        ...
        this.getPercent = function (off, newmas) {
            ...
            newmas = newmas || 0;
            if (off == false) {
                ceilMax = Math.ceil(this.maxElement());
                if (ceilMax < 100)
                ...
                ceilMax = Math.ceil(this.maxElement(true, newmas));
                if (ceilMax < 100)
                    ceilMax = (Math.round((ceilMax / 10)) + 1) * 10;
                else if (ceilMax >= 100)
                    ceilMax = (Math.round((ceilMax / 100)) + 1) * 100;
...
//------------------------------------End Draw box---------------------------------------------------//
    ...
    function DrawGraph(mat, colorGraph) {
        ...
        context.lineWidth = 3;
        for (var i = 0; i < 16; i++) {
            context.moveTo(relation[i][0], relation[i][1]);
            if (i != 15) context.lineTo(relation[i + 1][0], relation[i + 1][1]);
            context.stroke();
        }
        ...
        for (var j = 0; j < 16; j++) {
            ...
            context.moveTo(relation[j][0], relation[j][1] - 10);
            context.lineTo(relation[j][0], relation[j][1] + 10);
            context.stroke();

            context.moveTo(relation[j][0] - 5, relation[j][1] - 10);
            context.lineTo(relation[j][0] + 5, relation[j][1] - 10);
            context.stroke();
    ...
    imageCloseThird.onclick = function (e) {
        ...
        var barThird = document.getElementById("barThird"),
            opacity = 1;

        var interval = setInterval(function () {

            barThird.style.opacity = opacity;
            opacity = opacity - 0.1;
